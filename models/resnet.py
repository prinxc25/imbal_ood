...
class ResNet_DeepMCDD(nn.Module):
    def __init__(self, block_expansion=1, latent_size=512, num_classes=10):
        super(ResNet_DeepMCDD, self).__init__()
    ...
    def load_fe_weights(self, weights):
        self.fe.load_state_dict(weights)
    ...
    def _forward(self, x):
        out = F.relu(self.fe.bn1(self.fe.conv1(x)))
        out = self.fe.layer1(out)
        out = self.fe.layer2(out)
        out = self.fe.layer3(out)
        out = self.fe.layer4(out)
        out = F.avg_pool2d(out, 4)
        out = out.view(out.size(0), -1)
        return out
