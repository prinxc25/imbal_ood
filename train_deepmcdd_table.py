        ...
        for epoch in range(args.num_epochs):
            ...
            with torch.no_grad():
                ...
                for i, (data, labels) in enumerate(test_id_loader):
                    ...
                    predicted_id.append(predicted)
                    labels_id.append(labels)
                predicted_id = list(chain.from_iterable(predicted_id))
                labels_id = list(chain.from_iterable(labels_id))
                outfile = os.path.join(outdir, 'predicted_id.csv')
                f = open(outfile, 'w')
                for i in range(len(predicted_id)):
                    f.write("{}\n".format(predicted_id[i]))

                f.close()

                outfile = os.path.join(outdir, 'labels_id.csv')
                f = open(outfile, 'w')
                ...
                for i, (data1, labels1) in enumerate(test_ood_loader):
                    ...
                    predicted_ood.append(predicted1)
                    labels_ood.append(labels1)
                predicted_ood = list(chain.from_iterable(predicted_ood))
                labels_ood = list(chain.from_iterable(labels_ood))
                outfile = os.path.join(outdir, 'predicted_ood.csv')
                f = open(outfile, 'w')
                for i in range(len(predicted_ood)):
                    f.write("{}\n".format(predicted_ood[i]))
                f.close()
                outfile = os.path.join(outdir, 'labels_ood.csv')
                f = open(outfile, 'w')
                for i in range(len(labels_ood)):
                    f.write("{}\n".format(labels_ood[i]))
                f.close()
                # correct, total = 0, 0
