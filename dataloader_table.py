...
def get_table_data_with_iden_separate_sets(batch_size, data_dir, dataset, oodclass_idx, fold_idx, **kwargs):
    ...
    #-------------validation and test-----------------------
    data_path_val = os.path.join(data_dir,dataset, dataset + '_test.npy')
    ...
    for i in range(len(labels)):
        if labels[i] == oodclass_idx:
            ...
            labels[i] -= 1
    test_id_indices = [i for i in range(len(labels)) if labels[i] != -1]
    ood_indices = [i for i in range(len(labels)) if labels[i] == -1]

    test_id_dataset = TensorDataset(torch.Tensor(features[test_id_indices]),torch.LongTensor(labels[test_id_indices]))
    
    test_id_loader = DataLoader(dataset=test_id_dataset, batch_size=batch_size, shuffle=True, num_workers=1)
    
    test_ood_dataset = TensorDataset(torch.Tensor(features[ood_indices]),torch.LongTensor(labels[ood_indices]))
